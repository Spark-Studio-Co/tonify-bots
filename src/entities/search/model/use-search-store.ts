...
interface SearchState {
  ...
  updateFilter: <K extends keyof SearchFilters>(filterName: K, value: SearchFilters[K]) => void
  resetFilters: () => void
...
const mockAnnouncements: Announcement[] = [
  ...
  },
  {
    ...
    description: "2-комнатная квартира с хорошим ремонтом, вся мебель и техника",
    price: 45000,
...
export const useSearchStore = create<SearchState>((set, get) => ({
  ...
  performSearch: async () => {
    ...
    try {
      ...
      filteredResults = filteredResults.filter(
        (item) =>
          item.price !== undefined && item.price >= filters.priceRange[0] && item.price <= filters.priceRange[1],
      )
      ...
      if (filters.status !== "all") {
        filteredResults = filteredResults.filter(
          (item) => item.status === (filters.status === "active" ? "active" : "inactive"),
        )
      }
